...
This integration uses Aqua and SwapVM contracts. It does not use the 1inch
Swap API, Pathfinder, or Fusion. The AURKA solver calculates a bounded route
from the Space state; SwapVM then executes that route against the Space's Aqua
strategy.
...
3. The owner ships the strategy into Aqua. The strategy names the treasury and
   the allowed token balances and contains the SwapVM instructions.
...
5. The capacity epoch records how much value can be traded from the current
   portfolio state. A new epoch is opened after a trade or a price refresh.
...
2. The deterministic solver calculates a valid amount and creates the intent
...
the AURKA registries, the Sepolia router and helpers, the vault factory, and
the selected price oracle. In the current profile the oracle is
`MockPriceOracle`; Chainlink mode requires explicit feed addresses.
...
For another trade on the same deployment, refresh the mock prices and open a
new epoch instead of redeploying:
...
tokens, or initial oracle configuration changes. It creates new addresses;
the old deployment, vaults, strategies, and balances remain on Sepolia.
...
Redeployment does not migrate assets automatically. Recover assets from the
old owner-controlled Space first, then fund the replacement Space.
